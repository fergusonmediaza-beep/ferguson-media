...
/*
  Page-specific behaviour for /creative-hub/: the enquiry modal (now a
  themed Bootstrap Modal instead of a hand-rolled overlay — Bootstrap
  handles focus-trapping, backdrop, and Escape-to-close) and its
  EmailJS + reCAPTCHA v3 submission. Shared chrome (loader, cookie bar,
...
enquiryForm.addEventListener('submit', (e) => {
  ...
  const fullMessage = `Package / Service: ${pkg}\nBudget: ${budget}\n\n${message}`;

  grecaptcha.ready(() => {
    grecaptcha.execute(RECAPTCHA_SITE_KEY, { action: 'creative_hub_enquiry' }).then((token) => emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, {
      inquiry_type: pkg,
